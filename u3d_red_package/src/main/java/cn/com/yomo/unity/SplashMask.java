...
public class SplashMask {

    private ImageView mBgPic ;//过渡界面的背景图片
    private UnityPlayer mUnityPlayerView;
    private int mType = 0;//类型，根据业务来展示不同的背景图片
    private static SplashMask sInstance;//单例对象
     ...
     * @return
     */
    public static SplashMask getInstance(){
        if (null == sInstance){
            synchronized (SplashMask.class){
                if (null == sInstance) sInstance = new SplashMask();
            }
        }
        return sInstance;
    }
    ...
    public void onCreate(Bundle savedInstanceState,UnityPlayer unityPlayer,int type){
        this.mUnityPlayerView = unityPlayer;
        this.mType = type;
    ...
    private void showArRedBg() {
        ...
        mUnityPlayerView.addView(mBgPic,bgPicWidth,bgPicHeight);
    }
